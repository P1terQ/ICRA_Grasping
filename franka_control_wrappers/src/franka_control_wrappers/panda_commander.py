...
class PandaCommander(object):
    ...
    def set_named_poses(self):
        grip_ready_joint_values_as_dict = ([-0.012278128726616162, -0.7834047260627022, -0.0008402333314644925, -2.3615974901829904, -0.015725505022539034, 1.5662728869946938, 0.7658006468498042])
        # grip_ready_joint_values_as_dict = ([-0.1706, -0.8882, -0.8514, -1.9803, -0.6403, 1.3514, 0.6152])
        ...
        place_values_as_dict = ([0.8338639966099323, 0.7813643499006305, 0.6722988716175682, -1.6374551276209672, -0.6227699804272917, 2.1757760300636293, 0.5606248298370176])
        # grip_ready_joint_values_as_dict = ([-0.1706, -0.8882, -0.8514, -1.9803, -0.6403, 1.3514, 0.6152])
        place_values = [v for k, v in sorted(place_values_as_dict.items())]
        self.active_group.remember_joint_values('place', place_values)

        drop_box_joint_values_as_dict = ([ 0.8263,  1.0896,  0.8203, -0.8825, -0.7649,  1.7213,  2.2823])
